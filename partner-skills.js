...
const searchEl=document.getElementById("partner-search"),categoryEl=document.getElementById("partner-category"),activationEl=document.getElementById("partner-activation"),elementEl=document.getElementById("partner-element"),mountEl=document.getElementById("partner-mount"),effectEl=document.getElementById("partner-effect");
...
function matchesDetailedEffect(row,effect){if(!effect)return true;const desc=(row.skill.desc||"").toLowerCase();if(effect.startsWith("drop-")){const element=effect.slice(5);return row.tags.includes("drops")&&row.tags.includes(`boost-${element}`)&&/drop.*more items/.test(desc);}if(effect.startsWith("boost-"))return row.tags.includes(effect)&&!(row.tags.includes("drops")&&/drop.*more items/.test(desc));if(effect==="drops")return row.tags.includes("drops")||/drop.*more items/.test(desc);return row.tags.includes(effect);}
function filtered(){const q=searchEl.value.trim().toLowerCase(),category=categoryEl.value,activation=activationEl.value,element=elementEl.value,mount=mountEl.value,effect=effectEl.value;return partnerRows.filter(x=>(!q||x.pal.name.toLowerCase().includes(q)||x.skill.name.toLowerCase().includes(q)||(x.skill.desc||"").toLowerCase().includes(q))&&(!category||x.categories.includes(category))&&(!activation||x.activations.includes(activation))&&(!element||x.elements.includes(element))&&(!mount||x.mount===mount)&&matchesDetailedEffect(x,effect));}
...
[searchEl,categoryEl,activationEl,elementEl,mountEl,effectEl].forEach(el=>el.addEventListener(el===searchEl?"input":"change",render));
document.querySelector(".partner-quick-filters").addEventListener("click",event=>{const button=event.target.closest("[data-effect-shortcut]");if(!button)return;effectEl.value=button.dataset.effectShortcut;document.querySelectorAll("[data-effect-shortcut]").forEach(item=>item.classList.toggle("is-active",item===button&&button.dataset.effectShortcut!==""));render();});render();
