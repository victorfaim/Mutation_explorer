(function(root,factory){
  ...
  function createRenderer(ctx){
    ...
    const alphaLinks=marker=>{const id=marker.pal?.id,rows=[];if(marker.pal?.slug)rows.push({href:`pal.html?pal=${encodeURIComponent(marker.pal.slug)}&${langParam()}`,label:"Ver na Palpedia"});if(id){rows.push({href:`breeding.html?child=${encodeURIComponent(id)}&${langParam()}`,label:"Abrir Calculadora de Breeding"},{href:`caminho.html?target=${encodeURIComponent(id)}&${langParam()}`,label:"Abrir Caminho de Breeding"},{href:`reverso.html?target=${encodeURIComponent(id)}&${langParam()}`,label:"Abrir Mutação Reversa"});}return rows;};
