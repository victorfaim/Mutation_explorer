(function(root,factory){
  ...
  function loadDropTables(env,document){
    ...
    dropTablesPromise=new Promise((resolve,reject)=>{
      ...
      script.src="drop-tables-data.js?v=20260728-1";
  ...
  function createRenderer(ctx){
    ...
    const dropHtml=(drop,tables)=>{const meta=tables.items?.[drop.itemId],localizedName=meta?.names?.[env.PME_I18N?.locale||"pt-BR"],name=localizedName||meta?.name||drop.itemId,resolvedIcon=env.ITEM_ICON_MAP?.[drop.itemId]?.textureBasename||meta?.icon,icon=resolvedIcon?`<img src="assets/items/${encodeURIComponent(resolvedIcon)}.png" alt="">`:'<span class="public-map-drop-fallback" aria-hidden="true">◆</span>',href=meta?`item.html?id=${encodeURIComponent(drop.itemId)}&${langParam()}`:"",title=href?`<a href="${href}">${esc(tr(name))}</a>`:`<strong>${esc(tr(name))}</strong>`;return `<article class="public-map-drop">${icon}<div>${title}${drop.variants.map(row=>`<span>${tr("Chance")}: ${row.rate.toLocaleString(env.PME_I18N?.locale||"pt-BR")}% · ${tr("Quantidade")}: ${number(row.min)}${row.max!==row.min?`–${number(row.max)}`:""}</span>`).join("")}</div></article>`;};
